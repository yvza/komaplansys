...
$query = "SELECT count(id) as total FROM plansys WHERE review >= 3 AND review < 5 AND user_id = {$_SESSION['members_id']}";
array_push($arrHelper, $pdo->query($query)->fetch());

$query = "SELECT count(id) as total FROM plansys WHERE review >= 5 AND user_id = {$_SESSION['members_id']}";
array_push($arrHelper, $pdo->query($query)->fetch());
